...
//	std::cout << instr << " 0x" << std::hex <<address << std::endl;
	if(instr==0) {
		...
		if(SnoopResult==0 && NormalMode){              
			cout<<"Snoop result: NOHIT"<<endl;
		}

		else if(SnoopResult==1 && NormalMode){         
			cout<<"Snoop result: HIT"<<endl;
		}

		else if(SnoopResult==2 && NormalMode){                             
			cout<<"Snoop result: HITM"<<endl;
		}	
			...
			if (CACHE[temp_SET].LINE[curr_way].MESI == I) {			// Just Verify whether invalid
				if(SnoopResult==0){
					if(NormalMode){
						...
						cout<<"INITIATE BUS READ OPERATION"<<endl;
			...
			for(int j=0;j<ways;j=j+1){
				if (CACHE[i].LINE[j].MESI!=I) {
					if(NormalMode || SilentMode) {
						cout<<"Contents of cache(Tag) at Index "<< i << " way " << j << " is "<<CACHE[i].LINE[j].TAG <<endl;
					}
				}
			}
		}
